...
resetSelectState = (e) => {
  ...
  render() {
    let exactPath = (this.state.foundBreed.name) ? <Redirect to='/found' /> : (this.state.itemSelected ? <Redirect to={'/' + this.state.itemSelected} /> : <div>
    ...
    return (
      ...
      <Route path='/found' render={props=>(<div><Found breed = {this.state.foundBreed} resetFoundState={this.resetFoundState} /></div>)}></Route>
      {personalUrls}
    </div>
    </BrowserRouter>
    )
